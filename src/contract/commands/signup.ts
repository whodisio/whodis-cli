...
You have been successfully signed up!
  ...
  static flags = {
    ...
    email: flags.string({ char: 'p', description: 'the email address you would like to signup with' }),
  };
  ...
  async run() {
    ...
    // define the email
    const email = invokedFlags.email || (await cli.prompt('What email would you like to signup with?'));
